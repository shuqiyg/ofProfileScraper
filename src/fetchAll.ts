...
try {
  ...
  const collection = database.collection("users");
    ...
    const documentsArray = await existingDocuments.toArray();
    const accountNames = documentsArray.map(doc => doc.accountName);
    const outputFilePath = "mongoOfUsers.json";
    fs.writeFileSync(outputFilePath, JSON.stringify(accountNames));
